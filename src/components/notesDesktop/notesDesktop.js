...
import "./notesDesktop.css"; // Assuming it's a CSS file, correct the file extension
import enter from "../../images/icons/enter.png";
import ContentDesktop from "../contentDesktop/contentDesktop";
import useCustomContext from "../../states/useContext";
...
const NotesDesktop = () => {
  ...
  const { notes, setNotes, selected } = useCustomContext();
      ...
      <div className="desktop__notes__content">
        {notes && notes.length > 0
          ? notes.map((note, index) => (
              <ContentDesktop key={index} note={note} />
            ))
          : null}
      </div>
